...
export class Backend extends EventEmitter {
    ...
    public async getSession() {
        if (Date.now() < this.rateLimitedUntil) {
            console.log(`Esperando otros ${this.rateLimitedUntil - Date.now()}ms para verificar la sesión debido a la limitación de solicitudes.`);
            return;
        }
    ...
    public async sync() {
        ...
        if (Date.now() < this.rateLimitedUntil) {
            console.log(`Esperando otros ${this.rateLimitedUntil - Date.now()}ms antes de sincronizar debido a la limitación de solicitudes.`);
            return;
        }
