...
export default function UserOptionsTab(props: any) {
            ...
            try {
                ...
                reader.onload = (e) => {
                    ...
                    if (data.length > 0) {
                        ...
                        setErrorMessage("El archivo importado no contiene datos de chats.");
                    }
                };
                reader.readAsText(file);
            } catch (error) {
                setErrorMessage("No se pudieron importar datos de chats.");
            }
        },
        [doc]
    );
    ...
    const successMessage = importedChats ? (
        <div style={{ color: 'green' }}>
            ...
            <span style={{ marginLeft: '0.5rem', marginTop: '0.7rem' }}>{importedChats} Chat(s) importado(s)</span>
        </div>
    ) : null;

    const errorMessageElement = errorMessage ? (
        <div style={{ color: 'red', marginTop: '0.7rem' }}>{errorMessage}</div>
    ) : null;

    return (
        <SettingsTab name="user">
            <SettingsOption heading="Importar y exportar">
                <div>
                    <Button variant="light" onClick={handleExport} style={{
                        marginRight: '1rem',
                    }}>Exportar</Button>
                    <FileButton onChange={handleImport} accept=".json">
                        {(props) => <Button variant="light" {...props}>Importar</Button>}
                    </FileButton>
                </div>
                {successMessage}
                {errorMessageElement}
