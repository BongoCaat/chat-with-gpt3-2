...
export default function MessageInput(props: MessageInputProps) {
    ...
    const onSpeechStart = useCallback(async () => {
        ...
        if (denied) {
            onSpeechError(new Error('El permiso de voz no se otorgó'));
            return;
        }
        ...
        try {
            if (!recording) {
                ...
                if (useOpenAIWhisper || !supportsSpeechRecognition) {
                    ...
                    onSpeechError(new Error('No soportado'));
                }
            } else {
                if (useOpenAIWhisper || !supportsSpeechRecognition) {
                    await stopRecording();
            ...
            <div style={{
                ...
                {context.generating && (<>
                    <Button variant="subtle" size="xs" compact onClick={() => {
                        ...
                        <FormattedMessage defaultMessage={"Cancelar"} description="Etiqueta para el botón que se puede hacer clic mientras la IA genera una respuesta para cancelar la generación" />
                    </Button>
                            ...
                            <Popover.Dropdown>
                                <div style={{
                                    ...
                                    <p style={{
                                        ...
                                        Lo sentimos, ocurrió un error al intentar grabar audio.
                                    </p>
