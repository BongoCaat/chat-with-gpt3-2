...
export default function MessageComponent(props: { message: Message, last: boolean, share?: boolean }) {
    ...
    const getRoleName = useCallback((role: string, share = false) => {
        switch (role) {
            case 'user':
                if (share) {
                    return intl.formatMessage({ id: 'role-user-formal', defaultMessage: 'Usuario', description: "Etiqueta que se muestra arriba los mensajes escritos por el usuario (a diferencia de la IA) para la conversación pública compartida (tercera persona, formal)." });
                } else {
                    return intl.formatMessage({ id: 'role-user', defaultMessage: 'Yo', description: "Etiqueta que se muestra arriba los mensajes escritos por el usuario (a diferencia de la IA) en las propias sesiones de chat del usuario (primera persona)." });
                }
                break;
            case 'assistant':
                    ...
                    <div className="metadata">
                            ...
                            {({ copy, copied }) => (
                                <Button variant="subtle" size="sm" compact onClick={copy} style={{ marginLeft: '1rem' }}>
                                    <i className="fa fa-clipboard" />
                                        {copied ? <FormattedMessage defaultMessage="Copiado" description="Etiqueta para el botón Copiar a Clipboard después de una copia exitosa" />
                                        : <span><FormattedMessage defaultMessage="Copiar" description="Etiqueta para el botón Copiar a Clipboard" /></span>}
                                </Button>
                            )}
                        </CopyButton>
                        {typeof navigator.share !== 'undefined' && (
                            <Button variant="subtle" size="sm" compact onClick={() => share(props.message.content)}>
                                ...
                                <span>
                                    <FormattedMessage defaultMessage="Compartir" description="Etiqueta para un botón que comparte el texto de un mensaje de chat utilizando la funcionalidad de compartir del dispositivo del usuario" />
                                </span>
                            </Button>
                        )}
                            ...
                            <Button variant="subtle" size="sm" compact onClick={() => {
                                ...
                                <span>
                                    {editing ? <FormattedMessage defaultMessage="Cancelar" description="Etiqueta para un botón que aparece cuando el usuario está editando el texto de uno de sus mensajes, para cancelar sin guardar los cambios" />
                                        : <FormattedMessage defaultMessage="Editar" description="Etiqueta para el botón donde el usuario puede hacer clic para editar el texto de uno de sus mensajes" />}
                                </span>
                            </Button>
                        )}
                        {!context.isShare && props.message.role === 'assistant' && (
                            <Button variant="subtle" size="sm" compact onClick={() => context.regenerateMessage(props.message)}>
                                ...
                                <span>
                                    <FormattedMessage defaultMessage="Regenerar" description="Etiqueta para el botón utilizado para pedirle a la IA que regenere uno de sus mensajes. Dado que las generaciones de mensajes son estocásticas, el mensaje resultante será diferente." />
                                </span>
                            </Button>
                        )}
                    </div>
